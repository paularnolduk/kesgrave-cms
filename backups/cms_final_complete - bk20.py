...
def get_homepage_events():
    """Get upcoming events from Event table with direct CORS headers - FIXED"""
    ...
    try:
        ...
        print(f"🔍 DEBUG: Starting events API debug")
        print(f"🔍 DEBUG: Today's date is: {date.today()}")
        
        # Debug: Check Event table
        try:
            total_events = Event.query.count()
            ...
            for event in sample_events:
                print(f"🔍 DEBUG: Sample event: ID={event.id}, Title={event.title}, Start_Date={event.start_date}, Status={event.status}")
        except Exception as e:
            print(f"🔍 DEBUG: Error querying Event table: {e}")
        
        # Get events that are in the future and published
        today = date.today()
        print(f"🔍 DEBUG: Looking for events with start_date >= {today} and status = 'Published'")
        
        try:
            events_query = Event.query.filter(
                Event.start_date >= today,
                Event.status == 'Published'
            ).order_by(Event.start_date.asc()).limit(6)
            
            events_list = events_query.all()
            print(f"🔍 DEBUG: Found {len(events_list)} events matching criteria")
            
            for event in events_list:
                print(f"🔍 DEBUG: Matching event: ID={event.id}, Title={event.title}, Start_Date={event.start_date}, Status={event.status}")
        except Exception as e:
        ...
        print(f"🔍 DEBUG: Final events data: {events_data}")
...
def get_homepage_meetings():
    ...
    try:
        ...
        try:
            ...
            # Fallback: try to get meeting types directly from Meeting table
            try:
                meeting_types_raw = db.session.query(Meeting.meeting_type_id).distinct().all()
                print(f"🔍 DEBUG: Found {len(meeting_types_raw)} distinct meeting type IDs in Meeting table")
                meeting_types = []
                for mt_id_tuple in meeting_types_raw:
                    mt_id = mt_id_tuple[0]
                    print(f"🔍 DEBUG: Meeting type ID: {mt_id}")
            except Exception as e2:
                print(f"🔍 DEBUG: Error with fallback query: {e2}")
                meeting_types = []
        ...
        try:
            ...
            for meeting in sample_meetings:
                print(f"🔍 DEBUG: Sample meeting: ID={meeting.id}, Date={meeting.date}, Type_ID={meeting.meeting_type_id}")
        except Exception as e:
