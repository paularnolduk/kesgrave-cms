...
def get_homepage_slides():
    """
    Get slides for homepage slider from ContentPage with category 'homepage'
    Returns slides with featured status, images, titles, descriptions, and action buttons
    """
    try:
        # Query ContentPages with category 'homepage' and subcategory 'slider' or 'slides'
        slides_query = ContentPage.query.filter(
            ContentPage.category.ilike('%homepage%'),
            db.or_(
                ContentPage.sub_category.ilike('%slider%'),
                ContentPage.sub_category.ilike('%slides%'),
                ContentPage.sub_category.ilike('%banner%')
            )
        ).order_by(ContentPage.created_date.desc()).limit(5)
        
        slides_data = []
        for slide in slides_query:
            # Parse related links for action button
            action_button_text = None
            action_button_url = None
            
            if slide.related_links:
                try:
                    links = json.loads(slide.related_links)
                    if links and len(links) > 0:
                        first_link = links[0]
                        action_button_text = first_link.get('title', 'Learn More')
                        action_button_url = first_link.get('url', '#')
                except:
                    pass
            
            # Determine if slide is featured (you can customize this logic)
            is_featured = 'featured' in slide.title.lower() or 'important' in slide.title.lower()
            
            slide_data = {
                'id': slide.id,
                'title': slide.title,
                'description': slide.content[:200] + '...' if len(slide.content) > 200 else slide.content,
                'featured_image': slide.featured_image,
                'action_button_text': action_button_text,
                'action_button_url': action_button_url,
                'is_featured': is_featured,
                'created_date': slide.created_date.isoformat() if slide.created_date else None
            }
            slides_data.append(slide_data)
        
        # If no slides found, return sample data
        if not slides_data:
            slides_data = [{
                'id': 1,
...
def get_homepage_events():
    ...
    try:
        ...
        events_query = Event.query.filter(
            Event.date >= today
        ).order_by(Event.date.asc()).limit(6)
        ...
        for event in events_query:
            ...
            if event.categories:
                try:
                    categories = json.loads(event.categories) if isinstance(event.categories, str) else event.categories
                except:
                    categories = [event.categories] if event.categories else []
            
            # Parse related links for first featured link
            first_featured_link = None
            if event.related_links:
                try:
                    links = json.loads(event.related_links)
                    if links and len(links) > 0:
                        first_featured_link = links[0].get('url', f'/ktc-events/{event.id}')
                except:
                    pass
            
            if not first_featured_link:
            ...
            event_data = {
                ...
                'first_featured_link': first_featured_link
            }
...
def get_homepage_meetings():
    """
    Get next upcoming meeting for each meeting type
    Returns meeting type, date, time, and location
    """
    try:
        from datetime import datetime, date
        
        # Get all meeting types
        meeting_types = db.session.query(Meeting.type).distinct().all()
        
        meetings_data = []
        today = date.today()
        
        for meeting_type_tuple in meeting_types:
            meeting_type = meeting_type_tuple[0]
            ...
            if next_meeting:
                meeting_data = {
                    ...
                    'date': next_meeting.date.isoformat() if next_meeting.date else None,
                    'time': next_meeting.time.strftime('%H:%M') if next_meeting.time else None,
...
def get_homepage_quick_links():
    """
    Get quick links for homepage from ContentPage with category 'homepage' and subcategory 'quick-links'
    Returns title, description, URL, and button text
    """
    try:
        # Query ContentPages with category 'homepage' and subcategory 'quick-links'
        quick_links_query = ContentPage.query.filter(
            ContentPage.category.ilike('%homepage%'),
            db.or_(
                ContentPage.sub_category.ilike('%quick%'),
                ContentPage.sub_category.ilike('%links%'),
                ContentPage.sub_category.ilike('%quick-links%')
            )
        ).order_by(ContentPage.created_date.desc()).limit(8)
        
        quick_links_data = []
        for link_page in quick_links_query:
            # Parse related links for URL and button text
            url = '#'
            button_text = 'Learn More'
            
            if link_page.related_links:
                try:
                    links = json.loads(link_page.related_links)
                    if links and len(links) > 0:
                        first_link = links[0]
                        url = first_link.get('url', '#')
                        button_text = first_link.get('title', 'Learn More')
                except:
                    pass
            
            # Use content as description, truncated
            description = link_page.content[:150] + '...' if len(link_page.content) > 150 else link_page.content
        ...
        # If no quick links found, return sample data
        if not quick_links_data:
            quick_links_data = [
                {
                    'id': 1,
        ...
        return jsonify(quick_links_data)
    ...
    with app.app_context():
        ...
        predefined_types = [
            ...
            },
            {
                ...
                'description': 'Meetings for community and recreation matters',
                'color': '#2ecc71',
                'show_schedule_applications': False
            },
            {
                'name': 'Finance and Governance',
                'description': 'Meetings for finance and governance matters',
                'color': '#f39c12',
                'show_schedule_applications': False
            },
            {
                'name': 'Full Council Meetings',
            ...
            },
            {
                ...
                'description': 'Meetings for planning and development matters',
                'color': '#9b59b6',
...
def init_homepage_data():
    """Initialize default homepage data"""
    with app.app_context():
        # Create default logo settings if none exist
        if HomepageLogo.query.count() == 0:
            default_logo = HomepageLogo(
                logo_text='Kesgrave Town Council',
                is_active=True
            )
            db.session.add(default_logo)
        
        # Create default footer columns if none exist
        if HomepageFooterColumn.query.count() == 0:
            footer_columns = [
                {
                    'column_number': 1,
                ...
                },
                {
                    ...
                    'links': [
                        {'name': 'Planning Applications', 'url': '/planning'},
                        {'name': 'Council Meetings', 'url': '/meetings'},
                        {'name': 'Community Groups', 'url': '/community'},
                        {'name': 'Local Facilities', 'url': '/facilities'}
                    ]
                },
                {
                    'column_number': 3,
                    ...
                    'links': [
                        {'name': 'Council Tax', 'url': '/council-tax'},
                        {'name': 'Local History', 'url': '/history'},
                        {'name': 'Transport', 'url': '/transport'},
                        {'name': 'Emergency Info', 'url': '/emergency'}
                    ]
                }
            ]
            ...
            for col_data in footer_columns:
                column = HomepageFooterColumn(
                    ...
                    column_title=col_data['column_title']
                )
                db.session.add(column)
                db.session.flush()  # Get the ID
                
                for i, link_data in enumerate(col_data['links']):
