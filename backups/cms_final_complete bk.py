...
def content_dashboard():
    ...
    categories = [
        {'name': 'Planning Applications', 'count': 15, 'color': '#e74c3c'},
        {'name': 'Environmental Policies', 'count': 8, 'color': '#27ae60'},
        {'name': 'Community Services', 'count': 12, 'color': '#3498db'},
        {'name': 'Finance & Budget', 'count': 6, 'color': '#f39c12'},
        {'name': 'Transport & Roads', 'count': 9, 'color': '#9b59b6'},
        {'name': 'Housing & Development', 'count': 7, 'color': '#34495e'}
    ]
    
    # Sample recent pages
    recent_pages = [
        {'title': 'Planning Application Process', 'category': 'Planning Applications', 'status': 'Published', 'updated': '22/06/2025'},
        {'title': 'Environmental Impact Assessment', 'category': 'Environmental Policies', 'status': 'Published', 'updated': '21/06/2025'},
        {'title': 'Community Centre Booking', 'category': 'Community Services', 'status': 'Draft', 'updated': '20/06/2025'},
        {'title': 'Annual Budget 2025', 'category': 'Finance & Budget', 'status': 'Published', 'updated': '19/06/2025'},
        {'title': 'Road Maintenance Schedule', 'category': 'Transport & Roads', 'status': 'Published', 'updated': '18/06/2025'}
    ]
...
def content_pages_list():
    # Sample data showing how hundreds of pages would be managed
    # In a real system, this would come from the database with pagination
    ...
    # Sample pages data (simulating hundreds of pages)
    all_pages = []
    categories = ['Planning Applications', 'Environmental Policies', 'Community Services', 'Finance & Budget', 'Transport & Roads', 'Housing & Development']
    statuses = ['Published', 'Draft', 'Archived']
    
    # Generate sample pages
    for i in range(1, 158):  # 157 total pages
        category = categories[i % len(categories)]
        status = statuses[i % len(statuses)]
        
        pages_data = {
            'Planning Applications': [
                'Planning Application Process', 'Development Guidelines', 'Building Regulations', 'Conservation Areas',
                'Planning Committee Minutes', 'Appeal Procedures', 'Permitted Development Rights', 'Tree Preservation Orders'
            ],
            'Environmental Policies': [
                'Environmental Impact Assessment', 'Climate Change Strategy', 'Waste Management Policy', 'Green Spaces Plan',
                'Air Quality Monitoring', 'Biodiversity Action Plan', 'Sustainable Transport', 'Energy Efficiency'
            ],
            'Community Services': [
                'Community Centre Booking', 'Library Services', 'Youth Programs', 'Senior Citizens Support',
                'Sports Facilities', 'Community Events', 'Volunteer Opportunities', 'Local Charities'
            ],
            'Finance & Budget': [
                'Annual Budget', 'Council Tax Information', 'Financial Reports', 'Procurement Policy',
                'Grant Applications', 'Audit Reports', 'Investment Strategy', 'Budget Consultation'
            ],
            'Transport & Roads': [
                'Road Maintenance Schedule', 'Traffic Management', 'Parking Regulations', 'Public Transport',
                'Cycling Infrastructure', 'Road Safety', 'Winter Gritting', 'Street Lighting'
            ],
            'Housing & Development': [
                'Housing Strategy', 'Affordable Housing', 'Housing Allocations', 'Homelessness Support',
                'Development Plans', 'Housing Standards', 'Right to Buy', 'Housing Repairs'
            ]
        }
        
        page_titles = pages_data[category]
        title = f"{page_titles[i % len(page_titles)]} {((i-1) // len(page_titles)) + 1 if i > len(page_titles) else ''}"
        ...
        page = {
            ...
            'status': status,
            'author': 'Admin User',
            'created': f"{(i % 28) + 1:02d}/06/2025",
            'updated': f"{(i % 28) + 1:02d}/06/2025",
            'views': (i * 17) % 1000,
            'summary': f"Information about {title.strip().lower()} for Kesgrave residents and stakeholders."
        }
        
        # Apply filters
        if category_filter and page['category'] != category_filter:
            continue
        if status_filter and page['status'] != status_filter:
            continue
        if search_query and search_query.lower() not in page['title'].lower():
            continue
                ...
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" name="category">
                            ...
                            <option value="Planning Applications" {% if category_filter == 'Planning Applications' %}selected{% endif %}>Planning Applications</option>
                            <option value="Environmental Policies" {% if category_filter == 'Environmental Policies' %}selected{% endif %}>Environmental Policies</option>
                            <option value="Community Services" {% if category_filter == 'Community Services' %}selected{% endif %}>Community Services</option>
                            <option value="Finance & Budget" {% if category_filter == 'Finance & Budget' %}selected{% endif %}>Finance & Budget</option>
                            <option value="Transport & Roads" {% if category_filter == 'Transport & Roads' %}selected{% endif %}>Transport & Roads</option>
                            <option value="Housing & Development" {% if category_filter == 'Housing & Development' %}selected{% endif %}>Housing & Development</option>
                        </select>
                    </div>
                    <div class="col-md-3">
    ...
    ''', pages=pages, total_pages=total_pages, category_filter=category_filter, 
         status_filter=status_filter, search_query=search_query, pagination_html=pagination_html)
...
def add_content_page():
    if request.method == 'POST':
        ...
        link_new_tabs = request.form.getlist('link_new_tab[]')
        ...
        for i, title in enumerate(link_titles):
            if title.strip() and i < len(link_urls) and link_urls[i].strip():
                link = ContentLink(
                    ...
                    new_tab=f'link_new_tab_{i}' in link_new_tabs,
                    sort_order=i
                )
                db.session.add(link)
                ...
                <div class="card section-card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="long_description" id="longDescription" rows="10"></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                ...
                <div class="col-md-10 main-content">
                    <div class="p-4">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div>
                                <h2><i class="fas fa-tags me-2 text-primary"></i>Event Categories Management</h2>
                                <p class="text-muted">Manage event categories and their settings</p>
                            </div>
                            <button class="btn btn-primary"><i class="fas fa-plus me-2"></i>Add New Category</button>
                        </div>
                                        ...
                                        <div class="row text-center">
                                            <div class="col-6">
                                                <div class="border-end">
                                                    <h6 class="text-muted mb-1">Events</h6>
                        ...
                        <div class="text-center py-5">
                            ...
                            <h4 class="text-muted">No Event Categories</h4>
