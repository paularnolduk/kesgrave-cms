...
# Event Detail API Endpoint - Add this to your CMS file
    ...
    try:
        ...
        for download in event_downloads:
            downloads.append({
                ...
                'file_url': f"{request.url_root}uploads/events/downloads/{download.filename}" if download.filename else None
            })
        ...
        for photo in event_gallery:
            gallery.append({
                ...
                'image_url': f"{request.url_root}uploads/events/gallery/{photo.filename}" if photo.filename else None
            })
        ...
        event_data = {
            ...
            'featured_image': f"{request.url_root}uploads/events/{event.image_filename}" if event.image_filename else None,
...
def get_homepage_events():
    """Get upcoming events with categories and colors - ENHANCED VERSION"""
    ...
    try:
        ...
        print(f"🔍 DEBUG: Starting ENHANCED events API with categories")
        ...
        for event in events_list:
            ...
            event_data = {
                ...
                'featured_image': featured_image_url,
                'categories': categories,  # NEW: Category tags with colors
                'url': f'/ktc-events/{event.id}'
            }
            events_data.append(event_data)
        
        print(f"🔍 DEBUG: Final events data with categories: {len(events_data)} events")
        for event in events_data:
            print(f"🔍 DEBUG: Event '{event['title']}' has {len(event['categories'])} categories")
